...
export default {
	...
	recoveryRequestPost: async (req, res) => {
		...
		switch (data.method) {
			case 'email':
				...
				mailService.send.recoveryLink(
					accountService.actionLink.createRecoveryLink(account, PasswordResetChannels.EMAIL),
					data.email,
					res.locals
				);
			...
			case 'token':
				...
				res.redirect(
					accountService.actionLink.createRecoveryLink(account, PasswordResetChannels.RECOVERY_TOKEN)
				);
				break;
			default:
	...
	accountSetup: async (req, res) => {
		...
		return dashboardAuthRenderer.initAccountPage(res, false, {
