...
export default {
	...
	accountChangePasswordPost: async (req, res) => {
		...
		res.redirect(extendUrl(env.BASE_URL, 'account').href);
	},
	...
	accountChange2faPost: async (req, res) => {
		...
		auditService.appendAuditLog(
			req.account,
			data.secret ? AuditActionType.TWO_FACTOR_AUTH_ENABLED : AuditActionType.TWO_FACTOR_AUTH_DISABLED,
			req
		);

		accountService.twoFactorAuth.set(req.account, data.secret);
		res.redirect(extendUrl(env.BASE_URL, 'account', '2fa').href);
	},
	...
	accountRecoveryDeleteEmailPost: async (req, res) => {
		...
		res.redirect(extendUrl(env.BASE_URL, 'account', 'recovery').href);
	},
	...
	terminateInvitePost: async (req, res) => {
		...
		res.redirect(extendUrl(env.BASE_URL, 'invites').href);
	},
			...
			configuration: JSON.stringify(
				{
					...
					redirect_uris: ['https://example.com/done'],
					response_types: ['code'],
				},
				null,
				'  '
	...
	serviceAddPost: async (req, res) => {
		...
		res.redirect(extendUrl(env.BASE_URL, 'services').href);
	},
	...
	serviceEditSavePost: async (req, res) => {
		...
		res.redirect(extendUrl(env.BASE_URL, 'services', 'edit', newClientId).href);
	},
	...
	serviceEditDeletePost: async (req, res) => {
		...
		if (!errors.isEmpty()) {
			if (!clientId) {
				...
				return dashboardRenderer.manageService(
					req,
					res,
					clientId,
					{
	...
	manageUserUpdatePost: async (req, res) => {
		...
		res.redirect(extendUrl(env.BASE_URL, 'user', 'manage', data.id).href);
	},
	...
	manageUserDeletePost: async (req, res) => {
		...
		res.redirect(extendUrl(env.BASE_URL, 'users').href);
	},
	...
	sendUserAccountSetupEmailPost: async (req, res) => {
		...
		const link = accountService.actionLink.createSetupLink(
			accountService.find.withId(data.id),
			PasswordResetChannels.ADMIN,
			60 * 60 * 25 * 60
		); // 60 Days
	...
	sendUserAccountRecoveryEmailPost: async (req, res) => {
		...
		const link = accountService.actionLink.createRecoveryLink(
			accountService.find.withId(data.id),
			PasswordResetChannels.ADMIN
		);
