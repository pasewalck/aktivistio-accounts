...
export default {
	...
	getActionTokenEntry: (tokenType, token) => {
        ...
        `
			...
			.get(tokenType, token);
		if (entry) entry.payload = JSON.parse(entry.payload);
		return entry;
	},
	...
	getAccountWithUsername: (username) => {
		let result = db
			.prepare(
				'SELECT id, username, role_id as role, is_active as isActive, last_login as lastLogin FROM accounts WHERE username = ?'
			)
			.get(username);
		return result
			? new Account(result.id, result.username, result.role, result.isActive == 1, result.lastLogin)
			: null;
	},
	...
	getAccountWithId: (id) => {
		let result = db
			.prepare(
				'SELECT id, username, role_id as role, is_active as isActive, last_login as lastLogin FROM accounts WHERE id = ?'
			)
			.get(id);
		return result
			? new Account(result.id, result.username, result.role, result.isActive == 1, result.lastLogin)
	...
	getAccountPasswordHashWithUsername: (username) => {
		return db
			.prepare('SELECT password_hash as passwordHash FROM accounts WHERE username = ?')
			.pluck()
			.get(username);
	},
	...
	createAccount: (id, username, role, isActive = true) => {
		db.prepare('INSERT INTO accounts (id, username, role_id, is_active) VALUES (?, ?, ?, ?)').run(
			id,
			username,
			role,
			isActive ? 1 : 0
	...
	getAccountRecoveryById: (id) => {
		return db
			.prepare('SELECT recovery_email_hash as email,recovery_token_hash as token FROM accounts WHERE id = ?')
			.get(id);
	},
	...
	getAllAccounts: () => {
		return db
			.prepare(
				'SELECT id, username, role_id as role, is_active as isActive, last_login as lastLogin FROM accounts'
			)
			.all()
			.map(
	...
	linkInviteCodeToEmail: (inviteCode, emailFingerprint) => {
		db.prepare('INSERT INTO email_invite_requests (email_fingerprint, code) VALUES (?, ?)').run(
			emailFingerprint,
			inviteCode
		);
	},
