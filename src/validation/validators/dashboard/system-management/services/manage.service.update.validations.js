...
export default [
	...
	body('configuration')
		...
		.withMessage(localize('adapter.configuration.required'))
		.bail()
		.isString()
		.withMessage(localize('adapter.configuration.type_invalid'))
		.bail()
		.custom(async (value, { req }) => {
			var json = JSON.parse(value);
			if (!json['client_id']) throw new Error(req.__('adapter.client.id_required'));
